...
void init_voxmap_param(int _win_size)
{
    win_size = _win_size;
}
...
public:
  ...
  void acc_evaluate2(const deque<IMUST> &xs, int head, int end, Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT, double &residual)
  {
  ...
  void left_evaluate(const deque<IMUST> &xs, int head, int end, Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT, double &residual)
  {
    Hess.setZero(); JacT.setZero(); residual = 0;
    // vector<PointCluster> sig_tran(win_size);
    int l = 0;
  ...
  void left_evaluate_acc2(const deque<IMUST> &xs, int head, int end, Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT, double &residual)
  {
    Hess.setZero(); JacT.setZero(); residual = 0;
    int l = 0;
    PLM(4) T(win_size);
  ...
  void evaluate_only_residual(const deque<IMUST> &xs, double &residual)
  {
    residual = 0;
    vector<PointCluster> sig_tran(win_size);
...
public:
  ...
  void reorganize(VOX_HESS &voxhess, pcl::PointCloud<PointType> &pl_send, pcl::PointCloud<PointType> &pl_cent, deque<IMUST> &x_buf)
  {
...
public:
  ...
  void cut_func(int ci)
  {
    ...
    for(uint j=0; j<a_size; j++)
    {
      ...
      // 0 0 1 -----   1
      // 0 0 0 -----   0
      int leafnum = 4*xyz[0] + 2*xyz[1] + xyz[2]; // Binary encoding, pretty clever
  ...
  void recut(int win_count)
  {
    if(octo_state != 1)
    {
      ...
      octo_state = 1;
      // Clear sig_orig and sig_tran vector
  ...
  void to_margi(int mg_size, deque<IMUST> &x_poses, int win_count)
  {
    if(octo_state != 1)
    {
      if(!x_poses.empty())
  ...
  {
    if(octo_state != 1)  // octo_state = 0 or 2
    {
      int points_size = 0;
      for(int i=0; i<win_count; i++)
...
public:
  ...
  void marginalize(int mg_size, deque<IMUST> &x_poses, int win_count)
  {
...
public:
  ...
  double divide_thread_right(deque<IMUST> &x_stats, VOX_HESS &voxhess, deque<IMUST> &x_ab, Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT)
  {
    int thd_num = 4;
    double residual = 0;
  ...
  double divide_thread_left(deque<IMUST> &x_stats, VOX_HESS &voxhess, deque<IMUST> &x_ab, Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT)
  {
    int thd_num = 4;
    double residual = 0;
    Hess.setZero(); JacT.setZero();
  ...
  double only_residual(deque<IMUST> &x_stats, VOX_HESS &voxhess, deque<IMUST> &x_ab)
  {
  ...
  void damping_iter(deque<IMUST> &x_stats, VOX_HESS &voxhess)
  {
    ...
    deque<IMUST> x_stats_temp = x_stats;

    deque<IMUST> x_ab(win_size);
    x_ab[0] = x_stats[0];
    for(int i=1; i<win_size; i++)
    {
               ...
               Eigen::Vector3d t_Imu_Lidar = Eigen::Vector3d::Zero())
{
  ...
  for(PointType &p_c : pl_feat.points)
  {
    ...
    Eigen::Vector3d pvec_tran = x_key.R * (R_Imu_Lidar * pvec_orig + t_Imu_Lidar) + x_key.p;
