...
struct IMUST
{
  ...
  IMUST(double _t, const Eigen::Matrix3d &_R, const Eigen::Vector3d &_p, const Eigen::Vector3d &_v, const Eigen::Vector3d &_bg, const Eigen::Vector3d &_ba, const Eigen::Vector3d &_g = Eigen::Vector3d(0, 0, -Gravity)) : t(_t), R(_R), p(_p), v(_v), bg(_bg), ba(_ba), g(_g) {}
                  ...
                  Eigen::Vector3d t_Imu_Lidar = Eigen::Vector3d::Zero())
{
  ...
  {
    Eigen::Vector3d pvec(ap.x, ap.y, ap.z);
    pvec = xx.R * (R_Imu_Lidar * pvec + t_Imu_Lidar) + xx.p;
    ap.x = pvec[0];
    ap.y = pvec[1];
    ap.z = pvec[2];
  }
